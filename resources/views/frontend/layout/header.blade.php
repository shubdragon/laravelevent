...
@endphp
<nav class="navbar rounded m-0 bg-white z-30 shadow-md relative px-3">
    <div
        class="flex xl:space-y-0 xxsm:space-y-2 xmd:space-y-0 md:space-y-2 msm:flex-col md:flex-col xmd:flex-row xxsm:flex-col justify-between py-3 pt-4 z-30">
        <div class="flex items-center sm:items-left w-auto ">
            <ul
                class="navbar-nav flex lg:flex-row xmd:flex-row md:flex-row md:text-xs md:space-x-3 sm:flex-row msm:flex-col xsm:flex-col xxsm:flex-col msm:space-x-2 sm:space-x-2 md:mt-0">
                <li class="sm:hidden">
                ...
                <li class="mt-2 xxsm:max-sm:hidden nav-item {{ $activePage == 'home' ? 'active' : '' }} ">
                    <a href="{{ url('/') }}"
                        class="nav-link md:px-1 capitalize font-poppins font-normal text-sm leading-6 text-gray">{{ __('Home') }}</a>
                </li>
                <li class="mt-2 xxsm:max-sm:hidden nav-item {{ Request::is('all-events') ? 'active' : '' }}">
                    <a href="{{ url('/all-events') }}"
                        class="nav-link md:px-1 capitalize font-poppins font-normal text-sm leading-6 text-gray">{{ __('Events') }}</a>
                </li>
                <li class="mt-2 xxsm:max-sm:hidden nav-item {{ Request::is('all-category') ? 'active' : '' }} ">
                    <div class="relative inline-block text-left">
                        <a href="#"
                            class="nav-link md:px-1 capitalize font-poppins font-normal text-sm leading-6 text-gray flex focus:outline-none"
                                        ...
                                        @foreach ($category as $item)
                                            <div class="flex items-left justify-left">
                                                <a href="{{ url('/events-category/' . $item->id . '/' . $item->name) }}"
                                                    class="flex items-left text-sm font-poppins font-normal leading-5 {{ $catactive == $item->name ? ' text-primary capitalize p-2 bg-primary-light' : ' ' }}  capitalize p-2 hover:bg-primary-light rounded-md w-full">{{ $item->name }}</a>
                                            </div>
                                        @endforeach
                                    @endif
                                    <div class="flex items-left justify-left">
                                        <a href="{{ url('/all-category') }}"
                                            class="flex items-left text-sm font-poppins font-normal leading-5 {{ $catactive == 'all' ? 'text-primary capitalize p-2 bg-primary-light' : ' ' }} capitalize p-2 hover:bg-primary-light rounded-md w-full">{{ __('All categories') }}
                ...
                <li class="mt-2 xxsm:max-sm:hidden nav-item {{ $activePage == 'blog' ? 'active' : '' }}">
                    <a href="{{ url('/all-blogs') }}"
                        class="nav-link md:px-1 capitalize font-poppins font-normal text-sm leading-6 text-gray">{{ __('Blog') }}</a>
                </li>
                <li class="mt-2 xxsm:max-sm:hidden nav-item {{ $activePage == 'contact' ? 'active' : '' }} ">
                    <a href="{{ url('/contact') }}"
                        class="nav-link md:px-1 capitalize font-poppins font-normal text-sm leading-6 text-gray absolute" >{{ __('Contact Us') }}</a>
                </li>

            </ul>
            <div class="w-full text-center xxsm:max-sm:visible invisible">
                <button id="nav-toggle"
                    class="btn text-gray bg-white  text-left font-poppins font-normal
              text-sm leading-6 ">{{ __('Menu') }}</button>
            </div>
        </div>
        <div class="hidden" id="nav-content">
            <ul class="list-reset lg:flex justify-end flex-1 items-center text-end">
                ...
                <li class="mt-2 sm:hidden nav-item {{ $activePage == 'contact' ? 'active' : '' }}">
                    <a href="{{ url('/contact') }}"
                        class="nav-link md:px-1 capitalize font-poppins font-normal text-sm leading-6 text-gray">{{ __('contact Us') }}</a>
                </li>
            </ul>
        </div>
    <!--    <div class="ml-10 pl-5">-->
    ...
    <!--            <div class="relative">-->
            ...
            <!--        text-sm leading-6 rounded-md mx-1 focus:outline-none xxsm:w-full sm:w-48"-->
                                    ...
                                    <ul class="space-y-8 ">
                                        <li>
                                            <a href="{{ url('/my-tickets') }}"
                                                class="flex items-center font-normal font-poppins leading-6 text-black text-sm capitalize">{{ __('My tickets') }}
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ url('/user/profile') }}"
                                                class="flex items-center font-normal font-poppins leading-6 text-black text-sm capitalize">{{ __('Profile') }}
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ url('/change-password') }}"
                                                class="flex items-center font-normal font-poppins leading-6 text-black text-sm capitalize">{{ __('Change password') }}
                                            </a>
                                        </li>
                                    </ul>
                                </div>
    ...
    @else
        <div class="sm:px-4 flex xxsm:flex-wrap justify-end" id="signIn">
            <a type="button" href="{{ url('user/login') }}"
                class="px-5 py-3 text-white bg-primary text-center font-poppins font-normal text-sm leading-6 rounded-md">{{ __('Sign In') }}</a>
        </div>
    @endif
    </div>
</nav>
    ...
    document.getElementById("check").addEventListener('click', function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                document.location = "/public/user/logoutuser";
            }
        })
    });
</script>
